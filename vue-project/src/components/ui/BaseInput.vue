<template>
  <div :class="{'input-wrapper': type == 'file'}">
    <label :for="identity" class="fw-semibold mb-1">
      {{label}} <span style="color: #cb3a31">*</span>
      <slot></slot>
    </label>
    <input :class="[{ 'file-input': type == 'file'},'form-control']" :type="type" :id="identity" :placeholder="placeholder" :value="modelValue" :readonly="readonly === '1'" @input="$emit('update:modelValue', $event.target.value)" @keyup="$emit('keyInput', $event.target.value)" 
